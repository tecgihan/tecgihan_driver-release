...
## Software Configurations of IMS-SD Driver<br> - IMS-SD ドライバソフトウェア構成
...
1. **ims_sd_driver.py**
   ...
   - Linux software that does not depend on ROS  
     ROS には依存していない Linux ソフトウェア
   - [IMSSDDriverForRobot API](https://github.com/tecgihan/tecgihan_driver/blob/main/doc/ims_sd_driver.md#class-imssddriverforrobot)
   ...
   - ROS Topic Message Types - ROS トピックメッセージ型
     * `~/imu` : `sensor_msgs/Imu`
       + `header` : `Header`
         ...
         - `frame_id` : `string`
       + `linear_acceleration` : `Vector3` - [m/s²] (converted from [G])
       + `angular_velocity` : `Vector3` - [rad/s] (converted from [deg/s])
       + `orientation_covariance[0]` = `-1` (orientation not provided)
     * `~/mag` : `sensor_msgs/MagneticField`
       + `magnetic_field` : `Vector3` - [T] (converted from [uT])
...
## DMA-03 Running the Demonstration - DMA-03 デモンストレーションの実行
...
## How to Run IMS-SD - IMS-SD の実行方法

Connect USB Dongle to a USB port on your Ubuntu PC.  
USB Dongle を Ubuntu PC の USB ポートに接続します．
...
```bash
source /opt/ros/$ROS_DISTRO/setup.bash
ros2 launch tecgihan_driver ims_sd_ros_launch.py
```
...
```bash
$ ros2 topic list
/ims_sd_publisher/battery
/ims_sd_publisher/imu
/ims_sd_publisher/mag
/ims_sd_publisher/temperature
/parameter_events
/rosout
$
```
...
```bash
ros2 topic echo /ims_sd_publisher/imu
```

Press Ctrl-C to Quit.  
Ctrl-C を押して終了．

### Launch Options and Default Values - 起動オプションとデフォルト値
...
```bash
ros2 launch tecgihan_driver ims_sd_ros_launch.py debug:=true frequency:=500
```

- `debug:=false`
  * bool
  * Output informations to the console in "IMS-SD Driver" or not.  
    "IMS-SD ドライバ" でコンソールに動作情報を出力するか否か．
  * `true` : Driver console output - ドライバからのコンソール出力をする．
  * `false` : No driver console output - ドライバからのコンソール出力をしない．
- `timer:=-0.01`
  * double
...
- `frequency:=100`
  * int
  * Sampling frequency [Hz] - サンプリング周波数
  * `100` or `500` or `1000`
- `acc_range:=30`
  * int
  * Accelerometer full-scale range [G] - 加速度フルスケールレンジ
  * `4` or `8` or `16` or `30`
- `timeout:=1.0`
  * double
  * Serial port reading timeout [sec] - シリアルポート読み込み制限時間
- `serial_no:=''`
  * string
  * Set to distinguish between each one when using multiple IMS-SD units.  
    複数の IMS-SD 使用時に各個の IMS-SD を区別するために設定
  * Device Serial Number
  * If both this and the `location` option are empty strings, the first recognized IMS-SD is used.  
    `location` オプションもともに空文字列の場合は一番最初に認識された IMS-SD を使用する．
- `location:=''`
  * string
  * Set to distinguish between each one when using multiple IMS-SD units.  
    複数の IMS-SD 使用時に各個の IMS-SD を区別するために設定
...
- `frame_id:=imu_sensor`
  ...
  * Set `frame_id` for ROS TF (Transform) that performs robot frame coordinate transformation.  
    ROS のロボットフレーム座標変換を担う TF (Transform) 用にフレームIDを設定する．
- `node_name:=ims_sd_publisher`
  * string
  * ROS node name - ROS ノード名

### Using Multiple IMS-SD Units - IMS-SD の複数利用

When using multiple IMS-SD units, each device is identified with one of the following methods and launch the sensor driver.

複数の IMS-SD を利用する場合には次のいずれかの方法で各デバイスを識別してセンサドライバを起動します．

To find the serial number or device location of a connected IMS-SD, run the following command.  
接続されている IMS-SD のシリアルナンバーまたはデバイスロケーションを調べるには次のコマンドを実行します．

```bash
python -m serial.tools.list_ports -v | grep -B 1 -A 1 "FT230X Basic UART"
```

The following is an example of running with unique node names and frame IDs for two IMS-SD units.  
2つの IMS-SD を個別のノード名・フレームIDで起動する場合の例です．

**Terminal-1 - ターミナル-1**

```bash
ros2 launch tecgihan_driver ims_sd_ros_launch.py location:=1-2 node_name:=ims_sd_publisher_1 frame_id:=imu_sensor_1
```

**Terminal-2 - ターミナル-2**

```bash
ros2 launch tecgihan_driver ims_sd_ros_launch.py location:=1-1 node_name:=ims_sd_publisher_2 frame_id:=imu_sensor_2
```
...
The Linux driver scripts such as `dma03_driver.py` and `ims_sd_driver.py` in this package are designed to be ROS-independent.
Therefore, they can also be used to create non-ROS applications on Linux.

本パッケージの `dma03_driver.py` や `ims_sd_driver.py` などの Linux ドライバスクリプトは ROS に依存しないように作られています．
